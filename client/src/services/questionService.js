...
export const saveAnswers = async (questionId, queryItemId) => {
  if (questionId == null || queryItemId == null || Number.isNaN(Number(questionId)) || Number.isNaN(Number(queryItemId))) {
...
export const validateAnswersComplete = (questions, answers) => {
  ...
  if (unanswered.length > 0) {
    throw new Error(`未回答の質問があります（${unanswered.map((q) => q.title).join("、")}）。もう一度お試しください。`);
  }
};
