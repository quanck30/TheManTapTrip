...
function Home({ onDiagnoseComplete }) {
  const [currentStep, setCurrentStep] = useState(() => parseInt(localStorage.getItem('diag_step') || '0', 10));
  const [answers, setAnswers] = useState(() => JSON.parse(localStorage.getItem('diag_answers') || '{}'));
  const [isConfirming, setIsConfirming] = useState(() => localStorage.getItem('diag_isConfirming') === 'true');
  ...
  const isAllAnswered = diagnosticQuestions.every(q => answers[q.id]);
            ...
            <div className="summary-container">
              {diagnosticQuestions.map((q) => (
                <div key={q.id} className="summary-item">
                  ...
                  <p className="summary-answer">{answers[q.id]}</p>
                </div>
              ))}
            </div>
            <button className={`option-item confirm-btn ${isAllAnswered ? 'active' : ''}`} onClick={() => isAllAnswered && onDiagnoseComplete(answers)}>
              <span className="option-text">診断結果を見る</span>
            </button>
            <button className="back-button" onClick={handleBack}>← 修正する</button>
          </>
        ) : (
          <>
            ...
            <div className="question-meta">質問 {currentStep + 1} / {diagnosticQuestions.length}</div>
            <h2 className="question-title">{currentQuestion.question}</h2>
            <div className="options-group">
              {currentQuestion.options.map((option, index) => (
                <button key={index} className={`option-item ${answers[currentQuestion.id] === option ? 'selected' : ''}`} onClick={() => handleOptionSelect(option)}>
